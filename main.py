...
class LectureTracker:
    ...
    def show_progress_bar(self):
        ...
        bar_length = 20
        filled_length = int(
            bar_length * (self.total_lectures_listened / self.lecture_goal)
        )
    ...
    def set_lecture_goal(self):
        ...
        while True:
            ...
            if display_goal_less_than_listened_lectures_error_message:
                print(
                    f"{cc.RED}Lectures goal cannot be LESS than the amount of lectures completed so far!{cc.END}"
                )
                display_goal_less_than_listened_lectures_error_message = False

            if display_goal_equal_to_listened_lectures_error_message:
                print(
                    f"{cc.RED}Lectures goal cannot be EQUAL to the amount of lectures completed so far!{cc.END}"
                )
                display_goal_equal_to_listened_lectures_error_message = False

            try:
                print()
                self.lecture_goal = int(
                    input(f"Enter amount of lectures to complete: ")
                )
                clear_screen()
            except ValueError:
                clear_screen()
    ...
    def reset_streak(self):
        while True:
            ...
            proceed_input = (
                instant_input(
                    f"❌ You missed a day! Your streak has restarted 😣. ({cc.DARK_GRAY}p{cc.END}) proceed: "
                )
                .strip()
                .lower()
            )
    ...
    def error_msg_and_reset_streak_conditon(self):
        return (
            self.total_lectures_listened > 0
            and self.today_s_date() == self.date_to_reset_streak
        )
...
> """
    ...
    def get_valid_user_menu_choice(self):
        ...
        while True:
            ...
            if user_choice in self.VALID_OPTIONS:
                if (
                    user_choice == self.TURN_ON_STREAK_BASED_COUNTING
                    and self.total_lectures_listened > 0
                ):
    ...
    def if_lecture_goal_met(self):
        ...
        if self.lecture_goal == 1:
            ...
            print(
                f"Congrats on completing all {self.lecture_goal} of your lectures 🥳!"
            )
